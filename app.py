...
def load_and_prep_data():
    """
    Simula il caricamento di un dataset cross-section/panel.
    """
    np.random.seed(42)
    n = 2500
    ...
    # Rigore econometrico: Winsorizzazione
    ...
    # Reintegrato l'impatto dell'età nell'equazione latente
...
def calcola_pricing(prob_default, esg_score):
    """
    Trasforma il rischio in prezzo e applica il Greenium.
    """
    tasso_base = 3.50 # Es. Euribor 
...
# --- 4. INTERFACCIA WEB ---
# Tolta l'icona "banca" per un look più sobrio
...
with col1:
    ...
    # Reintegrata l'Età in cima alla prima colonna
    eta = st.number_input("Età Richiedente", 18, 90, 29)
...
with col2:
    ...
    ritardi = st.number_input("Ritardi Storici (Numero di rate)", 0, 10, 0)
    esg_score = st.slider("ESG Pillar Score (0-100)", 0, 100, 50, 1)
...
if st.button("Elabora Merito Creditizio", type="primary"):
    ...
    # Preparazione input per il modello (con Età)
    is_det = 1 if contratto == 'Determinato' else 0
    is_piva = 1 if contratto == 'Partita IVA' else 0
    
    # L'ordine delle variabili deve rispecchiare esattamente le features di addestramento
